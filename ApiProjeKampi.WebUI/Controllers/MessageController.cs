...
namespace ApiProjeKampi.WebUI.Controllers
{
    public class MessageController : Controller
    {
        ...
        public async Task<IActionResult> UpdateMessage(int id)
        {
            ...
            var value = JsonConvert.DeserializeObject<GetMessageByIdDto>(jsonDate);
            return View(value);
        }
        [HttpPost]
        ...
        [HttpGet]
        public async Task<IActionResult> AsnwerMessageWhitOpenAI(int id, string prompt)
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7041/api/Messages/GetMessage?id=" + id);
            var jsonDate = await responseMessage.Content.ReadAsStringAsync();
            var value = JsonConvert.DeserializeObject<GetMessageByIdDto>(jsonDate);
            prompt = value.MessageDetails;

            var apiKey = "";  // apkey openAI   
        ...
        public async Task<IActionResult> SendMessage(CreateMessageDto createMessageDto)
        {
            ...
            var jsonData = JsonConvert.SerializeObject(createMessageDto);
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client2.PostAsync("https://localhost:7041/api/Messages", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("MessageList");
            }
